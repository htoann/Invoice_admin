import { Button, ButtonPermission } from '@/components/button';
import { Modal } from '@/components/modal';
import { API_TAX_PAYER_EXCEL } from '@/service/apiConst';
import { dataService } from '@/service/dataService';
import { getI18n, ORG_LIST, PERMISSIONS } from '@/utils/index';
...
export const UploadFile = ({ getList }) => {
  ...
  const handleUpload = () => {
    ...
    dataService
      ...
      .then(() => {
        message.success(getI18n('Common_UploadFileSuccess', [file.name], false));
        setIsModalOpen(false);
        onReset();
        getList();
      })
      .catch(() => {
        message.error(t('Common_ImportExcelFailure'));
      })
      .finally(() => {
        setLoading(false);
      });
  ...
  return (
    <>
      <Upload {...uploadProps}>
        <ButtonPermission
          permissions={PERMISSIONS.TAXPAYER_IMPORT}
          type="primary"
          size="small"
          outlined
          icon={<UploadOutlined />}
      ...
      >
        ...
        >
          <span style={{ marginLeft: 10 }}>{t('Common_SelectAll')}</span>
        </Checkbox>
